...
BBO::BBO(const std::string &name)
      :
      RSAlgorithm(name, false),
      grid_(10) {
...
bool compare_shared_ptr(const std::shared_ptr<T> &a,
...
std::pair<bool, int> findInVector(const std::vector<T> &vecOfElements,
   ...
   auto it = std::find_if(vecOfElements.begin(), vecOfElements.end(),
                          [&element](const T &val) {
...
void BBO::doSort() {
   ...
   sort(sortedIdx.begin(), sortedIdx.end(), [&cost](size_t i1, size_t i2) {
...
//	}
   ...
   int j = 0;
   if (!(solutionsCosts.size() == solutions.size()
         && solutions.size() == assignedRider.size()
         && solutions.size() == unassignedRider.size()
         && solutions.size() == CandidateList.size()
         && solutions.size() == lookupVehicle.size())) {
...
bool BBO::checkSCH(int solutionIdx, MutableVehicleSptr const &r) {
   ...
   if ((r->schedule().data().size() != solutions[solutionIdx][r].size())
         || (r->schedule().data().size() != sum)) {
      print << "	CHK--OK" << std::endl;
      return 0;
   }
   return 1;
}
bool BBO::checkVehlStopsWithRiders(
         ...
         for (auto const &rider : obj.second) {
            ...
            for (auto const stop : obj.first->schedule().data())
               if (stop.owner() == rider.id())
                  sum++;
            if (sum != 2) {
               print << "Solution: " << solCnt << ", Vehl " << obj.first->id()
                     << "stops are not match with rider " << rider.id()
                     << std::endl;
               return 0;
            }
         }
...
bool BBO::checkVehileConsistencyInAllStructures(
   ...
   for (auto const &cand : solutions)
      if (cand.first != lookupVehl.at(cand.first->id())) {
         std::cout << "Vehl's pointer in solutions " << cand.first
                   << " not match with " << lookupVehl.at(cand.first->id())
                   << std::endl;
         return 0;
      }
   for (auto const &cand : assignedRider)
      if (cand.second != lookupVehl.at(cand.second->id())) {
         std::cout << "Vehl's pointer in solutions " << cand.second
                   << " not match with " << lookupVehl.at(cand.second->id())
                   << std::endl;
         return 0;
      }
   for (auto const &obj : CandidateList)
      for (auto const &cand : obj.second)
         if (cand != lookupVehl.at(cand->id())) {
            std::cout << "Vehl's pointer in solutions " << cand
                      << " not match with " << lookupVehl.at(cand->id())
                      << std::endl;
            return 0;
         }
   return 1;
}
bool BBO::checkSCHTemp(
   ...
   if (tempSolutions[solutionIdx].count(r) != 0)
      if (r->schedule().data().size() - 2
            != tempSolutions[solutionIdx].at(r).size() * 2) {
...
bool BBO::checkVehlStopsDuplication(const MutableVehicleSptr &vehl,
                                    const Customer &cust) {
   for (auto const &i : vehl->schedule().data())
      if (i.owner() == cust.id()) {
         print << "Rider " << cust.id() << " already exist in the vehl  "
               << vehl->id() << std::endl;
         return 1;
      }
...
bool BBO::PutMatchedHistory(const MutableVehicleSptr cand, const Customer &cust,
   ...
   if (is_matched == true)
      matchHist[std::make_tuple(cust, cand)] = std::make_tuple(stops,
                                                               is_matched, sch,
                                                               rte, cost);
   else {
      sch = { };
...
MutableVehicleSptr BBO::GetMatchedHistory(const MutableVehicleSptr cand,
                                          const Customer &cust,
                                          vec_t<Stop> &sch, vec_t<Wayp> &rte,
                                          DistInt &cost, bool &is_matched) {
   auto itr = matchHist.find(std::make_tuple(cust, cand));
   if (matchHist.end() != itr) {
      vec_t<TripId> stops = { };
      for (auto i : cand->schedule().data())
         stops.push_back(i.owner());
      if ((std::get<0>(itr->second) == stops)
            && (std::get<1>(itr->second) == true)) {
         sch = std::get<2>(itr->second);
         rte = std::get<3>(itr->second);
         cost = std::get<4>(itr->second);
         is_matched = true;
         return std::get<1>(itr->first);  //record is found
      } else if ((std::get<0>(itr->second) == stops)
            && (std::get<1>(itr->second) == false)) {
         is_matched = false;
         return std::get<1>(itr->first);  //record is found
      }
...
bool BBO::UpdateMatchedStructures(const uint8_t indx,
   ...
   if (indx >= assignedRider.size())
      ...
      print << "	Error in inserting a record to assignRider list: current cust is exist in the list"
            << std::endl;
      return false;
   }
   if (debugMode)
      print << "	Rider " << cust.id() << "is assigned to driver " << cand->id()
            << std::endl;
   return true;
}
...
MutableVehicleSptr BBO::initUsingGreedy(
   ...
   for (auto const cand : candidates) {
      ...
      if (GetMatchedHistory(cand, cust, sch, rte, new_cost, is_matched)
            != nullptr) {
         if (is_matched) {
...
MutableVehicleSptr BBO::initRandomely(const Customer cust,
   ...
   for (auto cand : candidates) {
      bool is_matched;
      if (GetMatchedHistory(cand, cust, sch, rte, cost, is_matched)
            != nullptr) {
         if (is_matched) {
            if (debugMode)
...
bool BBO::Greedy_Assignment(
      int solIndex, const Customer &cust,
   ...
   if (!match) {
      ...
      if (rollback_hist.count(best_vehl) == 0)
         rollback_hist[best_vehl] = std::make_tuple(
               best_vehl->schedule().data(), best_vehl->route().data());
...
//		checkSCHTemp(solIndex,best_vehl, tempAssignedRider, tempSolutions);
      if (debugMode)
         print << "	Rider " << cust.id() << " Assign to driver : "
               << temp_assignedRider[solIndex][cust]->id() << " with cost "
               << best_cost << " using Greedy" << std::endl;
   }
   return 1;
}

void BBO::ridersRandomlySelector(int indx, MutableVehicleSptr vehl,
...
void BBO::randXofY(uint8_t ridersCount, vec_t<Customer> &riders) {
...
void BBO::commit() {
   for (const auto &kv : solutions[0]) {
      ...
      if (this->assign(cadd, cdel, cand->route().data(),
                       cand->schedule().data(), *cand)) {
...
void BBO::solution_show() {
   for (int i = 0; i < PopulationSize; i++) {
      ...
      print << "	Solution: " << i << " Drivers: " << solutions[i].size()
            << ", Riders: " << solutions[i].size() << "\t";
...
bool BBO::migrate_vehicle_based(
      int popIndxSrc, int popIndxDest, const MutableVehicleSptr r,
   ...
   if (t_tempSolution[popIndxDest].count(copyVehl) == 0) {
      ...
      if (debugMode)
         print << "	Vehicle id " << copyVehl->id()
               << " is copied in the population " << popIndxDest << std::endl;
   ...
   if (debugMode)
      if (xx == 0) {
         print << popIndxSrc << ", " << popIndxDest << ", " << r->id()
               << std::endl;
         throw;
      }
   ...
   for (auto const &i : copyRiders) {
      std::pair<bool, int> result = findInVector<Customer>(alreadyAsssignRider,
                                                           i);
      if (debugMode)
   ...
   for (auto const &i : toBeMigratedRiders) {
      ...
      if (t_tempSolution[popIndxDest].count(cand)) {
         if (rollBackHist.count(cand) == 0)
            rollBackHist[cand] = std::make_tuple(cand->schedule().data(),
                                                 cand->route().data());
         //else
         //this cand exist in the hist
         t_tempSolution[popIndxDest][cand].erase(
               std::remove(t_tempSolution[popIndxDest][cand].begin(),
                           t_tempSolution[popIndxDest][cand].end(), i),
               t_tempSolution[popIndxDest][cand].end());
         if (t_tempSolution[popIndxDest][cand].size() == 0) {
            t_tempSolution[popIndxDest].erase(cand);
            if (debugMode)
               print << "	Driver " << cand->id() << "is removed from solution "
                     << popIndxDest << std::endl;
         }
...
//			checkSCHTemp(popIndxDest,cand, t_tempAssignedRider, t_tempSolution);
         if (debugMode)
            print << "	Rider " << i.id() << "is deassigned from " << cand->id()
                  << std::endl;
      } else
   ...
   for (const auto &i : reassignRiders) {
      ...
      if (t_tempSolution[popIndxDest].count(copyVehl)) {
         t_tempSolution[popIndxDest][copyVehl].erase(
               std::remove(t_tempSolution[popIndxDest][copyVehl].begin(),
                           t_tempSolution[popIndxDest][copyVehl].end(), i),
               t_tempSolution[popIndxDest][copyVehl].end());
         ...
         // Add the traveled distance to rte_after_rem <<<<<
         for (Wayp &wp : rte_after_rem)
            wp.first += copyVehl->route().dist_at(
                  copyVehl->idx_last_visited_node());
...
//			checkSCHTemp(popIndxDest,copyVehl, t_tempAssignedRider, t_tempSolution);
         if (debugMode)
            print << "	Rider " << i.id() << "is deassigned from "
                  << copyVehl->id() << std::endl;
      }
   }
   if (debugMode)
...
//		checkSCHTemp(popIndxDest,copyVehl, t_tempAssignedRider, t_tempSolution);
      if (debugMode)
         print << "	Rider " << i.id() << "is assigned to " << copyVehl->id()
               << std::endl;
   }
   if (debugMode)
   ...
   for (auto const &i : reassignRiders)
      if (!Greedy_Assignment(popIndxDest, i, t_tempSolution,
         ...
         if (rollBack) {
            if (debugMode)
               print << "	roll back becuase of rider " << i.id()
                     << " is unmatched!" << std::endl;
            commitChanges = 0;
         }
      }
...
void BBO::bbo_init() {
   ...
   for (uint8_t indx = 0; indx < PopulationSize; ++indx) {
      ...
      for (const auto &cust : customers) {
         vec_t<MutableVehicleSptr> candidates = local_grid[indx].within(
               pickup_range(cust), cust.orig());
      ...
      if (debugMode) {
         print << "	Solution " << (int) indx << ", Assigned Rider: "
               << assignedRider[indx].size() << ", Unassigned: "
               << unassignedRider[indx].size() << std::endl;
         checkVehileConsistencyInAllStructures(solutions[indx],
                                               assignedRider[indx],
                                               lookupVehicle[indx],
                                               CandidateList[indx]);
      }
...
void BBO::bbo_body() {
   if (debugMode)
      for (uint8_t i = 0; i < PopulationSize; ++i)
         checkVehileConsistencyInAllStructures(solutions[i], assignedRider[i],
                                               lookupVehicle[i],
                                               CandidateList[i]);
   ...
   generation_improvement[batchCounter - 1][0] = solutionsCosts[0];
   for (int Generation = 0; Generation < GenerationLimit; Generation++) {
      ...
      for (int k = 0; k < PopulationSize; ++k) {
         for (auto const &r : tempAssignedRider[k]) {
            ...
            if (rand() < mu[k]) {
               ...
               float Select = lambda[0];
               int SelectIndex = 0;  //destination solution
               while ((RandomNum > Select) && (SelectIndex < PopulationSize)) {
                  SelectIndex++;
                  Select += lambda[SelectIndex];
               }
               if (SelectIndex != k) {
                  if (debugMode)
                     print << std::endl << "From solution " << k << ", "
                           << r.second->id() << " -> " << r.first.id()
                           << " is transfering to solution " << SelectIndex
                           << std::endl;
                  if (!migrate_vehicle_based(k, SelectIndex, r.second,
                                             tempSolutions, tempAssignedRider,
                                             tempUnassignedRider))
                     if (debugMode)
                  ...
                  if (debugMode) {
                     ...
                     if (debugMode)
                        checkVehileConsistencyInAllStructures(
                              tempSolutions[k], tempAssignedRider[k],
                              lookupVehicle[k], CandidateList[k]);
                  }
               }
            }
         }
      ...
      if (debugMode)
         for (uint8_t i = 0; i < PopulationSize; ++i)
            checkVehileConsistencyInAllStructures(solutions[i],
                                                  assignedRider[i],
                                                  lookupVehicle[i],
                                                  CandidateList[i]);
      insertElits();
...
//		elites_grid={};
      ...
      generation_improvement[batchCounter - 1][Generation + 1] =
            solutionsCosts[0];
      if (debugMode) {
         for (uint8_t i = 0; i < PopulationSize; ++i)
            checkVehileConsistencyInAllStructures(solutions[i],
                                                  ...
                                                  lookupVehicle[i],
                                                  CandidateList[i]);
         print << "Minimum of the generation " << Generation << " is: "
               << generation_improvement[batchCounter - 1][Generation + 1]
               << std::endl;
      }
